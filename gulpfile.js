...
const rootJs = () =>
  gulp
    .src(path.src.rootJs, { base: srcPath })
    .pipe(
      plumber({
        errorHandler: function (err) {
          notify.onError({
    ...
    .pipe(gulp.dest(path.build.rootJs))
    .pipe(browserSync.reload({ stream: true }));
...
const watchFiles = () => {
  ...
  gulp.watch([path.watch.rootJs], rootJsWatch);
  gulp.watch([path.watch.js], jsWatch);
  gulp.watch([path.watch.img], images);
  gulp.watch([path.watch.fonts], fonts);
};

const build = gulp.series(clean, gulp.parallel(css, html, js, rootJs, images, fonts));
